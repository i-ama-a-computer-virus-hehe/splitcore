...
function snapshot(){return {type:'worldSnapshot',resources:serverResources,players:[...players.values()].map(p=>({...p,droneCount:(p.droneList||[]).length,drones:p.droneList||[]}))};}
const wss=new WebSocket.Server({server});wss.on('connection',ws=>{if(players.size>=12){ws.close(1013,'Server full');return}const id=nextId++;const spawn=spawnPoint();players.set(id,{id,name:'Unnamed Core',x:spawn.x,y:spawn.y,mass:0,drones:0,droneList:[],color:'#57a8ff'});ws.send(JSON.stringify({type:'welcome',id,maxPlayers:12}));ws.on('message',raw=>{try{const m=JSON.parse(raw);if(m.type==='name'){players.get(id).name=String(m.name||'Unnamed Core').slice(0,18)}if(m.type==='requestWorld'){ws.send(JSON.stringify(snapshot()));}
        if(m.type==='state'){Object.assign(players.get(id),m.state);
